...
#ifndef REGISTERBSPLINE_DISABLE_MAIN
...
int main(int argc, char **argv)
{
    if (argc < 11)
    {
        std::cout << "ACTestBSpline format.txt ref_image.png flo_image.png outname learning_rate symmetry_factor iterations sampling_fraction control_point_count gradient_magnitude_sigma" << std::endl;
        return -1;
    }
    ...
    double gradSigma = atof(argv[10]);
    const char *in_extra = (argc >= 12) ? argv[11] : 0;
    ...
    bool DoComputeDifferenceImages = false;
    ...
    IPT::SaveImageU16("./test.tif", movingImage);
    ...
    bool enable_gradient_magnitude = (gradSigma >= 0.0);
    ...
    for (unsigned int i = 0; i < levels; ++i)
    {
        ...
        ImagePointer maskPrime = maskUpd;
        if(subsamplingFactor[i] != 1) {
            ...
            maskPrime = IPT::SubsampleImage(maskUpd, subsamplingFactor[i]);
        }
