      ...
      IndexType index) const
  {
    ...
    while(true)
    {
      ...
      { // Continue traversing the tree
        ...
        /*
        IndexType midIndexLB = innerNodeInd;
        double rad = 0.0;
        for(unsigned int i = 0; i < ImageDimension; ++i) {
          midIndexLB[i] = midIndexLB[i] + innerNodeSz[i];
          double newrad = (0.5*innerNodeSz[i]+2)*spacing[i];
          if(newrad > rad)
          rad = newrad;
        }
        itk::Point<double, ImageDimension> innerNodePoint;
        image->TransformIndexToPhysicalPoint(midIndexLB, innerNodePoint);
        double lowerBoundDistance = corners[0].SquaredEuclideanDistanceTo(innerNodePoint) - rad*rad;
        */
