...
public:
  ...
  void Initialize()
  {
    ...
    // Initialize table
    m_Table.SetSize(m_SampleCount, CornersType::size);
...
private:
  ...
  void BuildTreeRec(unsigned int nodeIndex, IndexType index, SizeType sz)
  {
    ...
    NodeValueType *data = m_Array.data();
  ...
  void BuildTreeLoop(IndexType initIndex, SizeType initSz)
  {
    constexpr unsigned int dim = ImageType::ImageDimension;
    ...
    IndexType indices[128];
    SizeType sizes[128];
    unsigned int nodeIndices[128];
    bool dirDown[128];
    ...
    unsigned int curNodeIndex = 1;
    IndexType curIndex = initIndex;
    SizeType curSize = initSz;
    bool curDirDown = true;

    while (true)
    {
      if (curDirDown)
      {
        unsigned int szCount = PixelCount<dim>(curSize);
        if (szCount == 1U)
        {
          NodeValueType nv;
          nv[0] = m_Image->GetPixel(curIndex);
          nv[1] = m_One - nv[0];
          data[curNodeIndex - 1] = nv;

          if (stackIndex == 0)
            break;
          --stackIndex;
          curNodeIndex = nodeIndices[stackIndex];
          curIndex = indices[stackIndex];
          curSize = sizes[stackIndex];
          curDirDown = dirDown[stackIndex];
        }
        else
        {
          ...
          sz1[selIndex] = sz1[selIndex] / 2;
          sz2[selIndex] = curSize[selIndex] - sz1[selIndex];

          unsigned int nodeIndex1 = curNodeIndex * 2;
          unsigned int nodeIndex2 = curNodeIndex * 2 + 1;

          nodeIndices[stackIndex] = curNodeIndex;
          dirDown[stackIndex] = false;

          nodeIndices[stackIndex + 1] = nodeIndex1;
          sizes[stackIndex + 1] = sz1;
          indices[stackIndex + 1] = curIndex;
          dirDown[stackIndex + 1] = true;

          curNodeIndex = nodeIndex2;
          curIndex = midIndex;
          curSize = sz2;
          curDirDown = true;
      ...
      {
        unsigned int nodeIndex1 = curNodeIndex * 2 - 1;
        unsigned int nodeIndex2 = curNodeIndex * 2;

        NodeValueType n1 = *(data + nodeIndex1);
        NodeValueType n2 = *(data + nodeIndex2);

        NodeValueType *dataCur = data + (curNodeIndex - 1);
      ...
      IndexType index) const
  {
    ...
    while (true)
    {
      ...
      if (npx <= 0U)
      {
        ...
        RegionType region;
        region.SetIndex(curStackNode.m_Index);
        region.SetSize(curStackNode.m_Size);

        IteratorType it(image, region);
        it.GoToBegin();

        //std::cout << "From: " << curStackNode.m_Index << " with size: " << curStackNode.m_Size << std::endl;
        for (; !it.IsAtEnd(); ++it)
        {
          IndexType pind = it.GetIndex();
          //std::cout << pind << std::endl;
          ValueType val = it.Value();
          ValueType cval = m_One-val;

          unsigned int inEndLocal2 = PruneLevelsLinear(inwardsValues, inStartLocal, inEndLocal, val);
          unsigned int coEndLocal2 = PruneLevelsLinear(complementValues, coStartLocal, coEndLocal, cval);

          itk::Point<double, ImageDimension> leafPoint;
          for (unsigned int j = 0; j < ImageDimension; ++j)
            leafPoint[j] = static_cast<double>(pind[j] * spacing[j]);
          ...
          for (unsigned int i = 0; i < cornerCount; ++i)
          {
            ...
            for (unsigned int j = inEndLocal2; inStartLocal < j; --j)
            {
            ...
            {
            const double d = dists[i];
            for (unsigned int j = coEndLocal2; coStartLocal < j; --j)
            {
